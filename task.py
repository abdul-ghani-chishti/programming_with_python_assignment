...
mycursor_1 = db_connection.cursor()
    ...
    for y in y_train:
        if j < 4:
            for y_ideal in ideal_functions:
                ...
                if sum_squared_deviations < min_deviation:
                    min_deviation = sum_squared_deviations
                    best_function_params = params
            j = j+1
            best_function_val.append(best_function_params)
            min_sum_squared_deviations_arry.append(min_deviation)
    return best_function_val, min_sum_squared_deviations_arry
...
x_train, y_train = load_data("train.csv")  # training data (A)
ideal_functions = load_data("ideal.csv")  # ideal data (C)
x_test, y_test = load_data("test.csv")  # test data
...
for i, (x, y, best_fit_params, deviation) in enumerate(mappings):
    if best_fit_params is not None:
        ...
        data_to_insert = [(x, y[0], best_fit_params[0],
                           best_fit_params[1], deviation[0])]
        mycursor_1.executemany(
            "INSERT INTO mapping (x,y,ideal_x,ideal_y,deviation) VALUES (%s, %s, %s, %s, %s)", data_to_insert)
    else:
        print(f"Data point {
...
def plot_data_with_bokeh(x_train, y_train, x_test, y_test, chosen_functions, mappings):
    try:
        ...
        for i in range(y_train.shape[1]):
            p.scatter(x_train, y_train[:, i], legend_label=f'Training y{
                      i+1}', color='green', size=6, alpha=0.6)
        ...
        for i, params in enumerate(chosen_functions[0]):
            ...
            p.line(x_range, y_range, legend_label=f'Ideal Function {
                   i+1}', color='pink', line_width=2)

        # Plot test data
        for i in range(y_test.shape[1]):
            p.scatter(x_test, y_test[:, i], legend_label=f'Test Data y{
                      i+1}', color='black', size=6, alpha=0.6)
